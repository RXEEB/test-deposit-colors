...
export const Palette = ({ items, onRemoveColor ,setOpen}) => {
...

      {
        items.map((colors, id) => (
          <div onClick={() => setOpen}
            ...
            key={id}
            className='palette_box' style={{ background: colors.color }}>

            <svg className={`icon ${isHovering === id ? '' : 'hidden'}`}
              onClick={() => onRemoveColor(colors.id)}
